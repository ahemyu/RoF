...
public: 
  Talker(): Node("Talker"), counter_(0)
  {
    ...
    timer_ = this->create_wall_timer(
      50ms, std::bind(&Talker::timer_callback, this)); // create a timer that fires every 500 milliseconds, and when it fires, call the timer_callback() method on this object instance.
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<Talker>()); //spin calls call callback functions
  rclcpp::shutdown();
  return 0;
}
